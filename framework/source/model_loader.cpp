...
namespace model_loader
{
  ...
  // implemented for assignment 4 (normal maps)
  std::vector<glm::fvec3> generate_tangents(tinyobj::mesh_t const& model)
  {  
    ...
    for (unsigned i = 0; i < model.indices.size() / 3; i++)
    {
      ...
      glm::fvec3 v0 = positions[indices[0]];
      glm::fvec3 v1 = positions[indices[1]];
      glm::fvec3 v2 = positions[indices[2]];

      glm::fvec2 uv0 = texcoords[indices[0]];
      glm::fvec2 uv1 = texcoords[indices[1]];
      glm::fvec2 uv2 = texcoords[indices[2]];

      glm::fvec3 deltaPos1 = v1 - v0;
      glm::fvec3 deltaPos2 = v2 - v0;

      glm::fvec2 deltaUV1 = uv1 - uv0;
      glm::fvec2 deltaUV2 = uv2 - uv0;

      float r = 1.0f / (deltaUV1.x * deltaUV2.y - deltaUV1.y * deltaUV2.x);
      glm::fvec3 tangent = (deltaPos1 * deltaUV2.y - deltaPos2 * deltaUV1.y) * r;
      glm::fvec3 bitangent = (deltaPos2 * deltaUV1.x - deltaPos1 * deltaUV2.x) * r;
    ...
    {
      // orthogonalization
      glm::fvec3 t = tangents[i];
      glm::fvec3 n = normals[i];
      tangents[i] = t - n * glm::dot(n,t);
